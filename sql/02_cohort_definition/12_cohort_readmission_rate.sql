-- Readmission rate for FINAL COHORT ONLY
...
SELECT 
  COUNT(*) as total_discharges_in_cohort,
  SUM(CASE WHEN days_to_next_admission IS NOT NULL AND days_to_next_admission <= 30 THEN 1 ELSE 0 END) as readmissions_30d,
  ROUND(100.0 * SUM(CASE WHEN days_to_next_admission IS NOT NULL AND days_to_next_admission <= 30 THEN 1 ELSE 0 END) / COUNT(*), 2) as readmission_rate_cohort
FROM final_cohort;